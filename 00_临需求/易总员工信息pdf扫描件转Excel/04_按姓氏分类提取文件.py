...
exclude_names = {'景福', '北坝', '梓州', '观桥', '营业部',
    ...
    , '乐安'
    , '建平'
    , '办公室', '北路', '调查表', '附件3', '附件二', '计分', '南路', '签到表', '三台', '申报表', '统计', '统计表',
                 '西路', '新员工', '总部', '总行','中路'
                 }
# 确保目标文件夹存在
...
for filename in os.listdir(source_folder):
    if filename.lower().endswith(('.xls', '.xlsx')):
        ...
        if len(base_name) <= 3:
            ...
            # 获取拼音首字母
            surname_pinyin = lazy_pinyin(surname, style=Style.FIRST_LETTER)[0].upper()

            # 创建以拼音首字母命名的文件夹（如果不存在）
            target_folder = os.path.join(destination_folder, surname_pinyin)
